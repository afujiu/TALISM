<script>
  ...
  // 受信側ごとに異なるIDにする
  const myId = "viewer-" + Math.random().toString(36).substring(2, 8)
  ...
  async function receiveOffer(data) {
    ...
    pc.ontrack = async (event) => {
      console.log("映像受信:", event.streams[0])
    ...
    pc.onicecandidate = async (event) => {
      ...
      await sendCandidate(data.from, myId, event.candidate)
    }
    ...
    await sendAnswer(data.from, myId, pc.localDescription)
...
<div class="container">
  ...
  <button type="button" onclick={toggleFullscreen}> 全画面表示 </button>
  <button type="button" onclick={playVideo}> 映像を再生 </button>
  <button type="button" onclick={enableAudio}> 音声を有効化 </button>
  <button type="button" onclick={reconnect}> 再接続 </button>
  <video bind:this={videoElement} autoplay muted playsinline controls={false}
  ></video>
</div>
